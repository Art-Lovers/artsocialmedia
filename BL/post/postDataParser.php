...
class PostParser
{
    public static function createPostHtml($dbData, $profileId)
    {
        foreach ($dbData as $i => $data) {
            ...
            $htmlContent[$i] .= $img;
            $likeColor = (Post::isLikedByCurrent($data['postid'])) ? 'text-info' : 'text-dark';
            $htmlContent[$i] .= '<div class="d-flex justify-content-end socials p-2 py-3"><i class="bi bi-hand-thumbs-up-fill ' . $likeColor . ' likeButton">' . Post::updateLikeCount($data['postid']) . '</i><i class="fa fa-comments-o"></i><i class="fa fa-share"></i></div>';
